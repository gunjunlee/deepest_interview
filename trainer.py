    ...
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], input_scale_size=224, data_dir='/data/synthesized/model', model_save_name='model.ckpt', batch_size=64):
        ...
        self.data_transforms = {
                                    'train': transforms.Compose([
                                        transforms.Lambda(lambda img: self.convert_train(img)),
                                    ...
                                    'test': transforms.Compose([
                                        transforms.Lambda(lambda img: self.convert(img)),
                                        transforms.ToTensor(),
                                        transforms.Normalize(self.MEAN, self.STD)
                                    ])
                                }
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'test']:
                ...
                try:
                    ...
                    pdb.set_trace()
                print('{} Loss: {:.4f} Acc: {:.4f} {} Acc: {:.4f} {} Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc, self.class_names[0], epoch_acc_0, self.class_names[1], epoch_acc_1))
